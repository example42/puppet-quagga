...
# = Define: quagga::conf
#
# With this define you can manage any quagga configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/quagga/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/quagga/my.conf',
#
# [*ensure*]
...
define quagga::conf (
  ...
  $manage_path    = pickx($path, "${quagga::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $quagga::config_file_mode)
  $manage_owner   = pickx($owner, $quagga::config_file_owner)
  $manage_group   = pickx($group, $quagga::config_file_group)
  $manage_require = pickx($config_file_require, $quagga::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $quagga::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "quagga_conf_${name}":
    ensure  => $ensure,
    source  => $source,
